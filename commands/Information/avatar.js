...
module.exports = class AvatarCommand extends Command {
        ...
        super(client,
            {
                ...
                examples: [`${client.commandPrefix}avatar <@user/username/userid>`],
                args: [
                    {
                        key: "user",
    ...
    async run(message, {user}) {
        if(user.displayAvatarURL.includes(".png")){
            let embed = new Discord.RichEmbed()
                .setColor(`RANDOM`)
                .setAuthor(user.tag, user.displayAvatarURL)
                .setDescription(`[Avatar URL](https://cdn.discordapp.com/avatars/${user.id}/${user.avatar}.png?size=2048)`)
                .setImage(`https://cdn.discordapp.com/avatars/${user.id}/${user.avatar}.png?size=2048`)
                .setFooter(`Requested By: ${message.author.tag}`, message.author.displayAvatarURL)
            message.say(embed)
        } else 
        if(user.displayAvatarURL.includes(".jpg")){
            let embed = new Discord.RichEmbed()
                .setColor(`RANDOM`)
                .setAuthor(user.tag, user.displayAvatarURL)
