...
class CommandMessage {
	...
	async run() { // eslint-disable-line complexity
		...
		if(this.command.nsfw && !this.message.channel.nsfw) {
			...
			return this.reply(`The \`${this.command.name}\` command can't be used in this channel!`);
		}
		...
		try {
			...
			if(err instanceof FriendlyError) {
				...
				.setDescription(stripIndents`
					...
					Please contact ${ownerList || 'the bot owner'}${invite ? ` in this server: [Click Here](${invite})` : '.'}
				`)
				return this.say(embed4);
			}
		}
	}
